...
rm_header_ptr p_prev_used_node = &rm_used_list;	// points to last node on the used_list
...
rm_header_ptr insert_node(rm_header_ptr prev_node, void * v_addr, size_t s){
	rm_header_ptr curr_node = prev_node->next;
	rm_header_ptr new_node = create_node(v_addr, curr_node->size - s + sizeof(rm_header));
...
void call_more_page(size_t pg_size, rm_header_ptr end_node){
	...
	void * pg_front = mmap(0x0, pg_size / sysconf(_SC_PAGE_SIZE) + sysconf(_SC_PAGE_SIZE), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);

	// link to last node of free_list
	end_node->next = create_node(pg_front, pg_size - sizeof(rm_header) );

}

void free_to_used(rm_header_ptr free, rm_header_ptr used){
	used->next = free;
}
void * rmalloc (size_t s) 
{
	...
	if(rm_free_list.next == 0x0){
		...
		printf("init at %p\n", pg_free);
		rm_free_list.next = create_node( pg_free, pgsize - sizeof(rm_header));
	}
	...
	for(rm_header_ptr iter = rm_free_list.next ; iter != 0x0 ; iter = iter->next){
		...
		if(s == iter->size){
			printf("size == request\n");
			// switch free to used 
			p_prev_used_node->next = iter;
			p_prev_used_node->next->size = s;
		...
		if(s < iter->size){
			...
			// reduce the size by s + sizeof(node) and insert.
			return insert_node(p_prev_free_node, iter + sizeof(rm_header) + iter->size , iter->size - s);
		}

		// if s > pgsize and not found in iteration
		if(iter->next == 0x0){
			call_more_page(s, iter);
		}
